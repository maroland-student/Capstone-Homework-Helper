...
export async function handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (req.url?.startsWith('/api/openai/query')) {
        ...
        try {
            if (typeof image === 'string' && userPrompt.trim()) {
                ...
                ToggleLogs.log(`Sending image analysis prompt to OpenAI=${body.prompt}...`, LogLevel.INFO);
                OpenAIHandler.generateFromImage(body.prompt, body.image)
                    ...
                    .catch((err: any) => {
                        ToggleLogs.log(`Error generating from image: ${err}`, LogLevel.CRITICAL);
                        UrlUtils.sendJson(res, 500, {
                            error: 'Internal Server Error',
    ...
    try {
        UrlUtils.sendJson(res, 404, {
            error: 'Not Found',
            message: 'Unknown OpenAI endpoint.',
        });
        return;
    } catch (error: any) {
        let appError;
